...
function getCategoryIds() {
    ...
    let catId = response.data.map(c => c.id )
...
async function fillTable() {
    ...
    for (let catIdx = 0; catIdx < NUM_CLUES_PER_CAT; clueIdx++){
        let $tr = $("<tr>");
        for (let catidx = 0; catidx < NUM_CATEGORIES; clueIdx ++){
            $tr.append($("<td>").attr("id", `${catidx}-${clueIdx}`).text("?"));    
        }
        $("#jeopardy tobody").append($tr)
